      ...
      <section className="bg-surface py-space-xl">
        <div className="max-w-7xl mx-auto px-margin">
          <div className="inline-flex items-center gap-space-xs px-space-md py-1.5 rounded-full bg-surface-container text-primary text-label-sm font-display font-semibold uppercase tracking-wider mb-space-md shadow-card">
            ...
            OPD Live Today — GK-1 Clinic Open
          </div>
          ...
          <div className="mt-space-lg overflow-x-auto no-scrollbar">
            <div className="flex gap-space-xs min-w-max">
              {[
                { day: "MON", clinic: "Max Saket", hours: "10am – 1:30pm", isToday: false },
                { day: "TUE", clinic: "GK-1 Clinic", hours: "4:30pm – 8pm", isToday: true },
                { day: "WED", clinic: "Max Saket", hours: "10am – 1:30pm", isToday: false },
                { day: "THU", clinic: "GK-1 Clinic", hours: "4:30pm – 8pm", isToday: false },
                { day: "FRI", clinic: "Max Saket", hours: "10am – 1:30pm", isToday: false },
                { day: "SAT", clinic: "GK-1 / Medanta", hours: "Alt. Schedule", isToday: false },
                { day: "SUN", clinic: "Emergency Only", hours: "Max Saket ER", isToday: false, isEmergency: true },
              ].map((d) => (
                <div
                  key={d.day}
                  className={`flex-shrink-0 w-24 p-2.5 rounded-DEFAULT flex flex-col items-center text-center gap-0.5 ${
                    d.isEmergency
                ...
                >
                  <span className={`text-label-sm font-display font-bold ${d.isToday ? "text-primary-fixed-dim" : ""}`}>
                    {d.day}{d.isToday ? " • TODAY" : ""}
                  </span>
                  <span className={`text-label-md font-display font-extrabold ${d.isToday ? "text-inverse-on-surface" : ""}`}>{d.clinic}</span>
                  <span className={`text-label-sm ${d.isToday ? "text-inverse-on-surface/80" : "text-on-surface-variant"}`}>{d.hours}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-margin">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-space-lg">
            {doctor.clinics.map((clinic) => (
              <article key={clinic.id} className="bg-surface-container-lowest rounded-xl shadow-card p-space-lg flex flex-col gap-space-md">
                ...
                <div className="flex flex-col gap-space-xs pt-space-xs">
                  <Link href="/book" className="w-full flex items-center justify-center gap-space-xs py-3 rounded-full bg-primary-container text-on-primary-container text-label-md font-display font-bold hover:opacity-90 transition-all">
          ...
          <div className="mt-space-xl bg-surface-container-lowest rounded-xl shadow-card p-space-lg flex flex-col sm:flex-row items-center gap-space-md">
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              <p className="text-body-sm text-on-surface-variant">
                Chat with <strong className="text-on-surface">{doctor.coordinator.name}</strong>, {doctor.name}&apos;s Care Coordinator.
              </p>
            </div>
